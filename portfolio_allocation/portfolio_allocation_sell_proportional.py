"""
Portfolio Allocation Strategy - Proportional Sell Based on Buy Count

Uses buy/sell signals from combined_buy_sell_model to allocate portfolio.
Strategy: 
- 3% of capital per buy signal
- Sell percentage = 100% / (number of buys since last sell)
- If multiple sells in a row, continue using previous buy count
"""
import pandas as pd
import numpy as np
...
class PortfolioAllocator:
    """
    Portfolio allocator with proportional selling based on buy count
    
    Strategy:
    - 3% of total capital per buy trade
    - Track number of buy signals since last sell
    - On sell signal: sell (100% / buy_count) of position
    - If multiple sells in a row, continue using previous buy count
    - If no position on sell signal, reset buy counter to 0
    """
    ...
    def execute_sell_signal(self, date: str, ticker: str, price: float, signal_prob: float):
        ...
        Sell percentage = 100% / buy_count_since_last_sell
        - 1 buy -> sell 100%
        - 2 buys -> sell 50%
        - 3 buys -> sell 33.33%
        - etc.
        ...
        # Calculate sell percentage based on buy count
        if self.buy_count_since_last_sell == 0:
            # Edge case: no buys recorded, sell 100% (shouldn't happen normally)
            sell_percentage = 1.0
        else:
            # Sell percentage = 100% / buy_count
            sell_percentage = 1.0 / self.buy_count_since_last_sell
